  ...
  if ((NULL != cdimg_name) && (NULL != toc_name) && (NULL != cue_name)) {
    ...
    if (cdimg && toc && cue) {
      ...
      if (CD_OK == ret) {
        int trk_i = 1U;
        // White noise
        ret = write_noise(trk_i, &pos, cdimg, toc, cue, cdimg_name);
        trk_i++;
        // Square pulses
        if (CD_OK == ret)
        {
          ...
          {
            ret = write_track_square(trk_i, trk_p, &pos, cdimg, toc, cue, cdimg_name);
            if (CD_OK != ret) {
              break;
            }
          }
        ...
        if (CD_OK == ret)
        {
          for (size_t trk_p = 1; trk_p <= track_number_pulse; trk_p++, trk_i++)
          {
            ret = write_track_pulse(trk_i, trk_p, &pos, cdimg, toc, cue, cdimg_name);
            if (CD_OK != ret) {
              break;
            }
          }
        ...
        if (CD_OK == ret)
        {
          ...
          {
            ret = write_track_triangle(trk_i, trk_t, &pos, cdimg, toc, cue, cdimg_name);
            if (CD_OK != ret) {
              break;
            }
          }
...
int write_track_triangle(const int trk_i, const int trk_t, size_t *pos, FILE *cdimg, FILE *toc, FILE *cue, const char *dataname)
{
